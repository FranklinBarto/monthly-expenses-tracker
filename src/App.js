      ...
      <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              {backupStatus && (
                <div className={`mb-4 p-3 rounded-lg ${
                  backupStatus.includes('success') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
                }`}>
                  {backupStatus}
                </div>
              )}
        ...
        {view === 'track' && (
          <div className="space-y-6">
            ...
            {/* Add Expense Form */}
            <div className="bg-white rounded-lg shadow-lg p-6">
              <h2 className="text-xl font-bold text-gray-800 mb-4">Record New Expense</h2>
              <div className="grid gap-4 sm:grid-cols-5">
                <select
                  value={newExpense.categoryId}
                  onChange={(e) => setNewExpense({...newExpense, categoryId: e.target.value})}
                  className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent col-span-5 sm:col-span-1"
                >
                ...
                <button
                  onClick={addExpense}
                  className="w-full bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 col-span-5 sm:col-span-1"
                >
                  <PlusCircle size={20} />
                  Record
                </button>
              ...
              {!data.categories.length && (
                <p className="text-red-500 text-sm mt-3">**Please add a budget category in the Plan Budget view first.**</p>
              )}
            </div>
                        ...
                        .map(expense => {
                          ...
                          return (
                            <tr key={expense.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-xs truncate" title={expense.description}>{expense.description || '-'}</td>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="flex overflow-x-auto pb-4 space-x-4">
                {getHistoricalMonths(6).reverse().map((item, index) => (
              ...
              <div className="flex overflow-x-auto pb-4 space-x-4">
                {getHistoricalWeeks(8).reverse().map((item, index) => (
                  <HistoricalBar key={index} {...item} />
                ))}
              </div>
              ...
              <div className="flex overflow-x-auto pb-4 space-x-4">
                {getHistoricalDays(7).reverse().map((item, index) => (
                  <HistoricalBar key={index} {...item} />
                ))}
              </div>
            </div>
